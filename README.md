...
- It inventories small, visible, non-system always-on-top windows on the target display only after the exact EverQuest window is ready and before Magpie starts. Recognized companion surfaces such as Loremaster are also retained if they briefly drop their topmost flag during the focus handoff. Shell surfaces, system windows, the game, SpinFOURKAYYY, Magpie, and other fullscreen-sized windows are excluded.
- It watches native foreground and z-order changes and immediately returns those exact window handles above the verified scaling output when Magpie or an overlay changes order. A regular compatibility check remains as a fallback. Protection is active only while EverQuest, the owned Magpie process, or the companion itself is foreground; Alt+Tab to another application is left to the overlay's own normal policy.
